...
"""
crawled_data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../Crawl-n-Extract/Merge/UIUC.txt')

class XGBFacultyClassifier(object):
    def __init__(self, model_file='xgb.model'):
        # As we use same tf-idf vectorizer as used by our NN model, so loading the same here.
        self.vectorizer:TfidfVectorizer = pickle.load(open(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../data/improved_vectorized_data/vectorizer_object_new'), 'rb'))

        # Loading the saved model
        self.xgb_model = joblib.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), model_file))

    def predict(self, data, print_pred=False):
        print(f"Running predictions using XGBoost Classifier for {len(data)} pages")
        pp = PreProcessor() 
        processed_lines = []
        counter = 0
        data_len = len(data)
        ...
        predicted_values = self.xgb_model.predict(processed_line_vec)
...
if __name__ == "__main__":
    xgbClassifier = XGBFacultyClassifier()
    xgbClassifier.run_inference(sys.argv[1] if len(sys.argv) > 1 else crawled_data_path)